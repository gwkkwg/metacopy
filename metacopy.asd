...
;;; define the magic metacopy-file that will emit NAME.fasl and 
;; NAME-contextl.fasl
...
(defmethod output-files :around ((operation operation) (component metacopy-file-with-contextl)) 
  (with-contextl
	...
	(mapcar (lambda (path)
		  (push path result)
		  (push (merge-pathnames (concatenate 'string (pathname-name path) "-contextl") path) result))
		paths)
	result)))
...
(defsystem metacopy
  ...
  :components ((:module "dev"
                :components ((:static-file "notes.text")
                             ...
                             (:metacopy-file "contextl-integration" 
					     :depends-on ("package"))
                             (:metacopy-file "api" 
					     :depends-on ("contextl-integration" "package"))
                             (:metacopy-file "copy" 
...
;;; and the system connection that will load the entire metacopy
;;  code again into another package, :metacopy-with-contextl
...
(defmethod operation-done-p ((o test-op)
			     (c (eql (find-system 'metacopy-with-contextl))))
  nil)
...
(defmethod operate :around 
    ((o t) (c (eql (find-system 'metacopy-with-contextl)))
     &key &allow-other-keys)
  (with-contextl
    (call-next-method)))

(defmethod operate :around 
    ((o t) (c (eql (find-system 'metacopy-test-with-contextl)))
     &key &allow-other-keys)
  (with-contextl
    (call-next-method)))
